// Google Sheets Configuration - Using CSV export (no API key needed!)
...
async function fetchLessons() {
    try {
        ...
        for (let i = 0; i < rows.length; i++) {
            const row = rows[i];
            // Check if this row has actual content (not all empty)
            if (row.some(cell => cell && cell.trim() !== '')) {
                // Check if it looks like a header row (has "LESSON NAME" or "Type")
        ...
        const lessons = rows.slice(headerRowIndex + 1); // Skip header row

        displayLessons(lessons, headers);
...
function displayLessons(lessons, headers) {
    console.log('Headers:', headers);
    console.log('Lessons:', lessons);

    // Hide loading, show lessons container
    loadingEl.style.display = 'none';
    lessonsContainer.style.display = 'flex';

    // Find column indices (case-insensitive search)
    const typeIndex = headers.findIndex(h => h && h.toLowerCase().trim() === 'type');
    ...
    lessons.forEach(lesson => {
        ...
        if (type && type.trim() !== '') {
            lessonTypes.add(type.trim());
        }
    });

    console.log('Unique lesson types:', Array.from(lessonTypes));

    // Create a card for each lesson type
    Array.from(lessonTypes).forEach(type => {
        ...
        // Navigate to lesson type page with the type as parameter
        card.addEventListener('click', () => {
            window.location.href = `lesson-type.html?type=${encodeURIComponent(type)}`;
        });

        lessonsContainer.appendChild(card);
    });

    // If no lesson types were added
    ...
    if (searchInput) {
        searchInput.addEventListener('input', (e) => {
            ...
            if (searchTerm === '') {
                // Show all lesson types if search is empty
            ...
            lessons.forEach(lesson => {
                ...
                const lessonType = lesson[typeIndex] || '';
                ...
                if (lessonName.toLowerCase().includes(searchTerm) ||
                    lessonType.toLowerCase().includes(searchTerm) ||
            ...
            if (matchingLessons.length === 0) {
                ...
                matchingLessons.forEach(lesson => {
                    ...
                    card.innerHTML = `
                        ...
                        <p class="lesson-type">${escapeHtml(lesson.type)}</p>
                        ${lockOverlay}
                    `;
